...
export class Analyzer {
    ...
    public static SCRIPT_OPERATORS_REGEX = /(\+|-|\/|%|\*|<=|>=|=|<|>|\^| et | ou | mod )/;
                ...
                if (isMatch) {
                    ...
                    if (isMatch) {
                        ...
                        if (!startType.equals(endType))
                            this.scriptErrors.push(
                                new ScriptError("Le type de debut et de fin ne sont pas les memes", i, new Range(0, line.length))
                            );
                        else attrType = startType;
                    }
                }
